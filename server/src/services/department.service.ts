...
async function validateManagerForDepartment(
  managerId: string,
  departmentId: string
): Promise<void> {
  ...
  const managerStatus = manager.status as string;
  ...
  if (departmentId && manager.department_id !== departmentId) {
    throw ApiError.badRequest(
      "Manager harus berasal dari department yang sama dengan department yang dikelola."
    );
  }
  ...
  if (existingManager && existingManager.id !== departmentId) {
    throw ApiError.conflict(
      "Karyawan tersebut sudah menjadi manager di department lain."
    );
  }
}
    ...
    if (input.manager_id) {
      await db.transaction(async tx => {
        ...
        await tx
          ...
          .set({ position: "MANAGER" as Position, updated_at: new Date() })
    ...
    if (input.manager_id !== undefined || input.name !== undefined) {
      return db.transaction(async tx => {
        ...
        if (oldManagerId && oldManagerId !== newManagerId) {
          await tx
            .update(employeesTable)
            .set({ position: "STAFF" as Position, updated_at: new Date() })
